...
namespace YOLOv4.DataStructures
{
    public class YoloV4Prediction
    {
        ...
        public IReadOnlyList<YoloV4Result> GetResults(
            string[] categories,
            float scoreThres = 0.5f,
            float iouThres = 0.5f)
        {
                ...
                for (int boxY = 0; boxY < outputSize; boxY++)
                {
                    for (int boxX = 0; boxX < outputSize; boxX++)
                    {
                        for (int a = 0; a < anchorsCount; a++)
                        {
                            ...
                            {
                                postProcesssedResults.Add(
                                    new float[] { orgX1, orgY1, orgX2, orgY2, scoreMaxCat, scores.IndexOf(scoreMaxCat) });
                            }
                        }
                    }
                }
        ...
        /// <summary>
        /// Return intersection-over-union (Jaccard index) of boxes. <para>Both sets of boxes are expected to be in (x1,
        /// y1, x2, y2) format.</para>
        /// </summary>
        static float box_area(float[] box) { return (box[2] - box[0]) * (box[3] - box[1]); }
        ...
        public async Task<List<YoloV4Result>> GetResultsAsync(
            string[] categories,
            float scoreThres = 0.5f,
            float iouThres = 0.5f,
            System.Threading.CancellationToken token = default)
        {
            ...
            // Non-maximum Suppression
            postProcesssedResults = postProcesssedResults.OrderByDescending(x => x[4]).ToList(); // sort by confidence
            return await fillYoloV4Result(categories, iouThres, postProcesssedResults);
        }

        private async Task<List<YoloV4Result>> fillYoloV4Result(
            string[] categories,
            float iouThres,
