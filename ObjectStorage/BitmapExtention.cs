...
namespace ObjectStorage
{
    public static class BitmapExtention
    {
        ...
        public static BitmapSource ToWpfBitmap(this byte[] byteArrayIn)
        {
            if (byteArrayIn is null)
            {
                throw new ArgumentNullException(nameof(byteArrayIn));
            }

            using (var memstr = new MemoryStream(byteArrayIn))
            {
                Image img = Image.FromStream(memstr);
                return ((Bitmap)img).ToWpfBitmap();
            }
        }
